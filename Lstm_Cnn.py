...
class Lstm_CNN(object):
    ...
    def __init__(self):
        ...
        self.mask = tf.placeholder(tf.int32, shape=[None], name='mask')
    ...
    def lstm_cnn(self):
        ...
        with tf.name_scope('LSTM'):
            ...
            mask = tf.cast(tf.sequence_mask(lengths=self.mask, maxlen=pm.seq_length), tf.float32)
            mask = tf.expand_dims(mask, -1)
            seq_size = mask.shape[1].value
            Mask = []
            for k in range(seq_size):
                ones = mask[:, k, :] * tf.ones(shape=[pm.hidden_dim])
                Mask.append(ones)
            Mask = tf.concat(Mask, axis=1)
            Mask = tf.reshape(Mask, shape=[-1, pm.seq_length, pm.hidden_dim])
            outputs = output * Mask
